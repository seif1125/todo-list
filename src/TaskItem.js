...
const TaskItem = ({ task, }) => {
 ...
 useEffect(() => {
  ...
  const onSave = () => {
    // Ensure both title and description are non-empty and have been changed
    if (
      taskTitle.trim() &&
      taskDescription.trim() &&
      (taskTitle !== task.title || taskDescription !== task.description)
    ) {
      handleEditTask({ ...task, title: taskTitle, description: taskDescription });
    }
      ...
      <div className="task-bar">
        ...
        <div className="flex-class">
            ...
            <div className="task-actions">
              {!editMode && (
                <div className="complete-toggle">
                  <span className={`${task.completed ? 'completed' : ''}`}>{task.completed?'Completed':'in progress'}</span>
                  <input type="checkbox" onChange={()=>handleEditTask({ ...task,completed:!task.completed})} checked={task.completed} />
                </div>
              )}
              ...
              {!editMode && (
                <>
                  ...
                  <button onClick={()=>handleDeleteTask(task.id)} className="delete-task-button" aria-label="Delete Task">
                    {/* Delete Icon */}
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="16"
          ...
          {!editMode ? (
            ...
            <button onClick={onSave} className="task-item-toggle-description" aria-label="Save Task">
              Save
            </button>
          )}
        </div>
